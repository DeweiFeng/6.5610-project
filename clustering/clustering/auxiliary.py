...
def query_true_label(n_cluster, label_clustering, neighbors):
    ...
    # return np.array([create_vector_distribution(n_cluster, label_clustering[i_doc]) for i_doc in neighbors], dtype=np.int8)
    
    return np.array([create_vector_distribution(n_cluster, label_clustering[int(i_doc.item())]) for i_doc in neighbors], dtype=np.int8)
